...
class App extends Component {
  ...
  validate = () => {
    ...
    if (
      ...
      errors.firstName = 'First name must be between 2 and 12 letters long'
    }
    if (
      (this.state.touched.lastName && this.state.lastName.length < 3) ||
      (this.state.touched.lastName && this.state.lastName.length > 12) 
    ) {
      errors.lastName = 'Last name must be between 2 and 12 letters long'
    }
    return errors
  }
  handleSubmit = (e) => {
  ...
  render() {
    ...
    const { firstName, lastName } = this.validate() // because this.validate() returns an error object
    return (
      <div className='App'>
        <h3>Add Student</h3>
